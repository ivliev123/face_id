...
#2
	...
	if(len(faceList) <= array_data[i][9]):
		...
		for f in range(len(faceList)):
			...
			if len(temporal_dist_array[f])>3:
				faceList[f][1]=rectangle_now[array_namber[f][0]][0]
				faceList[f][2]=rectangle_now[array_namber[f][0]][1]
				array_data[array_namber[f][0]][10]=faceList[f][10]  
				faceList[f][5]=array_data[array_namber[f][0]][5] 

				for n in range(len(temporal_dist_array[f])):
			
			#временно обновляемданные во избежания конфликтации	
